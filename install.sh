...
port_alterid_set() {
    if [[ "on" != "$old_config_status" ]]; then
        ...
        [[ -z ${port} ]] && port="443"
        read -rp "Please enter alterID (default:0 allows only numbers) :" alterID
        [[ -z ${alterID} ]] && alterID="0"
    fi
}
modify_path() {
...
nginx_install() {
    ...
    echo -e "${OK} ${GreenBG} Compilation and installation are about to start jemalloc ${Font}"
    sleep 2
    ...
    echo -e "${OK} ${GreenBG} About to start compiling and installing Nginx, the process will take a while, please be patient ${Font}"
    sleep 4
...
domain_check() {
    ...
    if [[ $(echo "${local_ip}" | tr '.' '+' | bc) -eq $(echo "${domain_ip}" | tr '.' '+' | bc) ]]; then
        ...
        echo -e "${Error} ${RedBG} Please make sure that the correct A record is added to the domain name, otherwise V2ray will not work properly ${Font}"
...
port_exist_check() {
    if [[ 0 -eq $(lsof -i:"$1" | grep -i -c "listen") ]]; then
        ...
        lsof -i:"$1"
        echo -e "${OK} ${GreenBG} 5s Will try to kill the occupied process automatically ${Font}"
        sleep 5
...
old_config_exist_check() {
    if [[ -f $v2ray_qr_config_file ]]; then
        echo -e "${OK} ${GreenBG}If an old configuration file is detected, whether to read the old file configuration [Y/N]? ${Font}"
        read -r ssl_delete
        case $ssl_delete in
        [yY][eE][sS] | [yY])
        ...
        *)
            ...
            echo -e "${OK} ${GreenBG} Old configuration removed  ${Font}"
            ;;
        esac
    fi
}
...
enable_process_systemd() {
    ...
    if [[ "$shell_mode" != "h2" ]]; then
        ...
        judge "Set Nginx to auto-start at boot"
    fi
...
tls_type() {
    if [[ -f "/etc/nginx/sbin/nginx" ]] && [[ -f "$nginx_conf" ]] && [[ "$shell_mode" == "ws" ]]; then
        ...
        echo "Note that if you use Quantaumlt X / router / old version Shadowrocket / lower than 4.18.1 version V2ray core Please select Compatibility Mode"
...
ssl_update_manuel() {
    [ -f ${amce_sh_file} ] && "/root/.acme.sh"/acme.sh --cron --home "/root/.acme.sh" || echo -e "${RedBG}The certificate issuing tool does not exist, please confirm whether you are using your own certificate${Font}"
...
menu() {
    update_sh
    echo -e "\t V2ray Install management scripts ${Red}[${shell_version}]${Font}"
    ...
    17)
        ...
        ;;
    18)
        read -rp "Please enter the camouflage path (note! No need to add slashes eg:ray):" camouflage_path
        modify_camouflage_path
        start_process_systemd
        ;;
    *)
